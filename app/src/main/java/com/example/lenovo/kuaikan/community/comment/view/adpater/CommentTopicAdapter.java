...
public class CommentTopicAdapter extends RecyclerView.Adapter<CommentTopicAdapter.CommenterViewHolder> {
    ...
    @Override
    public void onBindViewHolder(CommenterViewHolder holder, final int i) {
        holder.tvUseName.setText(mComments.get(i).getRoot().getUser().getNickname());
        holder.mImageView.setRounImage(mComments.get(i).getRoot().getUser().getAvatar_url());
        //转化时间格式 MM-dd HH:mm
        String date = DateUtil.formatLongToDates(mComments.get(i).getRoot().getCreated_at()*1000);
        holder.mTvCreatTime.setText(date);
        holder.mTvCreatDetails.setText(mComments.get(i).getRoot().getContent());
        holder.mTvLikeNum.setText(mComments.get(i).getRoot().getLikes_count()+"");
